...
- 4 ✅ **Skeletor**: supervillain from 1980s franchise Masters of the Universe. *(AI is right 6/6)*
- 5 ❌ **Guy Fawkes mask**: (aka Anonymous mask), a symbol of protest and rebellion (0/6).
- 6 ⚠️ **Tombstone 272**: depicting the number of the artwork. *(AI is right 3/6)*
- 7 ⚠️ **Anarchy symbol**: approx. since 1970s. *(AI is right 1/6)*
...
Also I wanted to have a public repo in GitHub with third party libraries (e.g. `tenacity` for retries with backoff, `pydantic` for the output format) and standard techniques (e.g. `semaphores` from `asyncio` to limit concurrency) common to projects with unreliable external services/networks/dependencies like AIs/LLMs. Previous versions got the job done and were simpler, and the improvements reported in the issues tab are not strictly necessary.
